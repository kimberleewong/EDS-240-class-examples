...
ggplot(data = penguins,
       mapping = aes(x = bill_length_mm, y = bill_depth_mm, col = species)) +
  geom_point() 
```
...
ggplot(data = penguins,
       mapping = aes(x = bill_length_mm, y = bill_depth_mm, col = species)) +
  geom_point() +
  scale_color_manual(values = c("firebrick", "darkorange", "dodgerblue4"))
...
ggplot(data = penguins,
       mapping = aes(x = bill_length_mm, y = bill_depth_mm, col = body_mass_g)) +
  geom_point() +
  scale_color_gradient(low = "dodgerblue4", high = "salmon1")
```
...
ggplot() +
  ...
  geom_point(data = penguins_summary,
             aes(x = mean_bill_length_mm, y = mean_bill_depth_mm, fill = species),
             size = 6, shape = 24)

# You put the data locally (in the geom argument) instead of ggplot because we are visualizing multiple datasets
```
...
# When we map color by species on a global level, then all the subsequent layers are done in those groups
```
...
# Move the color argument to just the geom_point argument
```
...
ggplot(penguins, aes(x = flipper_length_mm, fill = species)) +
  ...
  labs(x = "Flipper Length (mm)", y = "Frequency",
       title = "Penguin Flipper Lengths", fill = "Species") +
